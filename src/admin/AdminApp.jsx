...
function Dashboard({ stats, recent }) {
  const cards = [
    ...
    ["Active services", stats.services, Zap, "customer solutions"],
    ["Media assets", stats.gallery, FileImage, "gallery records"]
  ];
  return <div className="dashboard-view">
    ...
    <div className="dashboard-grid"><section className="analytics-card"><div className="panel-title"><div><span className="admin-kicker">Enquiry pulse</span><h2>Last seven days</h2></div><BarChart3 /></div><div className="bars">{stats.trend.map((value, index) => <div key={index}><span style={{ height: `${Math.max(10, value * 18)}%` }} /><small>{["M","T","W","T","F","S","S"][index]}</small></div>)}</div></section><section className="recent-card"><div className="panel-title"><div><span className="admin-kicker">Latest activity</span><h2>Recent enquiries</h2></div></div>{recent.length ? recent.map((item) => <article key={item.id}><span>{item.name?.slice(0, 1)}</span><div><strong>{item.name}</strong><small>{item.service}</small></div><em>{item.status}</em></article>) : <div className="admin-empty">New enquiries will appear here.</div>}</section></div>
  </div>;
}
...
function Editor({ config, row, close, onSaved }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      await saveRow(config.table, payload); onSaved();
  ...
  })}</div>{error && <div className="admin-error">{error}</div>}<footer><button type="button" className="admin-secondary" onClick={close}>Cancel</button><button className="admin-primary" disabled={busy}>{busy && <LoaderCircle className="spin" />} Save changes</button></footer></form></div>;
}

function ModuleManager({ moduleKey }) {
...
function EnquiryManager() {
  ...
  const load = async () => setRows(await listRows("enquiries"));
  useEffect(() => { load(); }, []);
  const filtered = rows.filter((row) => (status === "all" || row.status === status) && JSON.stringify(row).toLowerCase().includes(search.toLowerCase()));
  const setRowStatus = async (row, value) => { await saveRow("enquiries", { ...row, status: value }); load(); };
  return <div className="module-view"><div className="module-heading"><div><span className="admin-kicker">Customer pipeline</span><h1>Enquiries</h1><p>Search, qualify and follow every customer conversation.</p></div></div><div className="module-toolbar"><label><Search /><input placeholder="Search enquiries..." value={search} onChange={(e) => setSearch(e.target.value)} /></label><select value={status} onChange={(e) => setStatus(e.target.value)}><option value="all">All statuses</option><option value="new">New</option><option value="contacted">Contacted</option><option value="qualified">Qualified</option><option value="closed">Closed</option></select></div><div className="enquiry-table">{filtered.map((row) => <article key={row.id}><div><strong>{row.name}</strong><a href={`tel:${row.phone}`}>{row.phone}</a><a href={`mailto:${row.email}`}>{row.email}</a></div><div><span>{row.service}</span><p>{row.message}</p><small>{new Date(row.created_at).toLocaleString()}</small></div><select value={row.status} onChange={(e) => setRowStatus(row, e.target.value)}><option value="new">New</option><option value="contacted">Contacted</option><option value="qualified">Qualified</option><option value="closed">Closed</option></select></article>)}</div></div>;
}
